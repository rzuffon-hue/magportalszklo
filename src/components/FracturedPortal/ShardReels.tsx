...
export const ShardReels: React.FC<ShardProps> = ({
  ...
  const { setActiveView, reels, playShardSound, portalTheme } = useApp();
    ...
    >
      ...
      {isComic ? (
        <div className="absolute inset-0 z-0">
          <ComicIllustration type="reels" className="opacity-90 transition-transform duration-300 group-hover:scale-105" />
        ...
        <div
          ...
          style={{
            background: isMirror
              ...
              : 'radial-gradient(circle at 70% 30%, rgba(225, 29, 72, 0.22) 0%, rgba(15, 12, 28, 0.25) 70%, rgba(3, 5, 12, 0.35) 100%)',
      ...
      <div
        className={`absolute inset-0 pointer-events-none transition-opacity ${
          isMirror
            ? 'bg-gradient-to-tr from-white/70 via-slate-200/30 to-slate-400/40'
            : 'bg-gradient-to-tr from-rose-950/15 via-slate-950/25 to-black/40'
        }`}
        style={{
          opacity: isShattered ? 1 : isShattering ? 1 : 0,
          transitionDuration: isShattering ? '500ms' : '300ms',
      ...
      >
        <GlassRimHighlight clipPath={activePolygon} themeColor={isMirror ? 'rgba(0, 0, 0, 0.8)' : 'rgba(244, 63, 94, 0.4)'} />
      </div>
      ...
      <div
        className={`absolute inset-0 pointer-events-none transition-opacity duration-150 z-20 ${
          isMirror
            ...
            : 'bg-gradient-to-tr from-white/35 via-rose-300/30 to-transparent'
        } ${isTouched ? 'opacity-100' : 'opacity-0'}`}
      />

      {/* 5. Micro-crack Internal Refraction Lines */}
      <svg className={`absolute inset-0 w-full h-full pointer-events-none ${
        isMirror ? 'opacity-40 stroke-slate-950' : 'opacity-15 stroke-rose-200/40'
      }`} viewBox="0 0 300 400">
      ...
      >
        <EtchedGlassIcon
          ...
          reflectionClass={isMirror ? 'text-black' : 'text-rose-300/80 group-hover:text-rose-100'}
          isTouched={isTouched}
        />
      </div>
      ...
      <div className={`absolute -bottom-10 -right-10 w-36 h-36 rounded-full pointer-events-none transition-all ${
        isMirror ? 'bg-white/80 blur-2xl' : 'bg-rose-600/15 blur-3xl group-hover:bg-rose-500/25'
      }`} />
    </div>
  );
};
