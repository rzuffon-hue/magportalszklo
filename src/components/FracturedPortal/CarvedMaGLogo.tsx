    ...
    <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-10 overflow-hidden select-none">
      <div
        className={`relative flex items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          isMirror
            ? (isShattered || isShattering ? 'opacity-[0.22] scale-100' : 'opacity-[0.28] scale-100')
            : (isShattered || isShattering ? 'opacity-[0.15] scale-100' : 'opacity-[0.13] scale-100')
        }`}
        style={{
          maskImage: 'radial-gradient(ellipse at center, rgba(0,0,0,1) 75%, rgba(0,0,0,0.4) 95%, transparent 100%)',
          WebkitMaskImage: 'radial-gradient(ellipse at center, rgba(0,0,0,1) 75%, rgba(0,0,0,0.4) 95%, transparent 100%)'
        }}
      >
        {/* Giant Monolithic Carved Text Relief Submerged Deep Beneath Glass Sheet */}
        <div className="relative font-black tracking-tighter text-center flex items-center justify-center text-[140px] sm:text-[230px] md:text-[340px] lg:text-[460px] xl:text-[540px]">
          
          {/* Deep Chiseled Base Groove */}
          <span
            className={`bg-clip-text text-transparent transition-colors duration-500 ${
              isMirror
                ? 'bg-gradient-to-b from-white via-slate-200 to-sky-300 drop-shadow-[0_4px_25px_rgba(255,255,255,0.9)]'
                : 'bg-gradient-to-b from-slate-100 via-slate-300 to-slate-600 drop-shadow-[0_10px_35px_rgba(255,255,255,0.2)]'
            }`}
            style={{
              fontFamily: "'Cinzel Decorative', 'Trajan Pro', 'Cinzel', 'Playfair Display', serif",
              letterSpacing: '-0.06em',
              WebkitTextStroke: isMirror ? '1.5px rgba(148, 163, 184, 0.45)' : '1px rgba(255, 255, 255, 0.25)'
            }}
          >
            MaG
          </span>
          ...
          <span
            className={`absolute inset-0 bg-clip-text text-transparent pointer-events-none ${
              isMirror
                ? 'bg-gradient-to-br from-white/80 via-sky-100/30 to-transparent'
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 overflow-hidden">
            <div
              className={`w-1/2 h-full blur-md transform -skew-x-25 ${
                isMirror
